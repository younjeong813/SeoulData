...
# 강좌 목록
BASE_DIR = os.path.abspath("./static/icon/page3")
VIDEO_DIR = os.path.abspath("C:/Users/User/Desktop/SKKU/2024 Winter Break/Microsoft Hackerton")

topics = [
    {"title": "이메일 보내기", "description": "이메일을 효과적으로 보내는 법을 배웁니다.", 
     "image": os.path.join(BASE_DIR, "email.jpg"),
     "details": "이메일 발송/수신과 관련된 기초적인 기능을 다룹니다.",
     "video": os.path.join(VIDEO_DIR, "ExampleVid-SendEmail.mp4")},
    {"title": "인터넷 뱅킹", "description": "안전하게 인터넷 뱅킹을 사용하는 법을 배웁니다.", 
     "image": os.path.join(BASE_DIR, "bank.jpg"),
     "details": "인터넷 뱅킹 로그인 및 송금 방법을 다룹니다.",
     "video": os.path.join(VIDEO_DIR, "ExampleVid-SendEmail.mp4")},
]

# 검색 기능
query = st.text_input("검색", placeholder="원하는 강좌를 검색하세요.", label_visibility="collapsed")
st.write("---")
...
if "selected_topic" in st.session_state:
    ...
    if os.path.exists(selected["video"]):
        st.video(selected["video"])
    else:
        st.warning("비디오를 찾을 수 없습니다.")
